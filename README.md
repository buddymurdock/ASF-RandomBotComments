...
С вероятностью `EmoticonChancePercent` к выбранному комментарию добавляется случайное эмодзи, которое отправляющий бот **реально владеет** (получено крафтом бейджей и т.п.) — код эмодзи (например, `:Blisk:`) берётся напрямую из его инвентаря (Steam-предметы типа `Emoticon`, appID 753), никакого стороннего/придуманного списка. Если у бота нет ни одного эмодзи, комментарий уходит как есть, без эмодзи.
...
```json
{
	...
	"RandomBotCommentsEmoticonChancePercent": 0,
...
| `RandomBotCommentsEmoticonChancePercent` | `byte`, 0–100 | `0` | Шанс добавить к комментарию реально принадлежащее боту эмодзи (см. выше). `0` — эмодзи никогда не добавляются. |
